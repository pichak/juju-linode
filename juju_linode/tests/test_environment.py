...
from juju_linode.env import Environment
...
class EnvironmentTest(Base):
    ...
    def test_bootstrap_jenv(self, run_juju):
        ...
        def verify_home(cmd, env, stderr=None):
            ...
            self.assertTrue(env['JUJU_HOME'].endswith('boot-linode'))
            with open(os.path.join(env['JUJU_HOME'],
                                   'environments.yaml')) as fh:
                data = yaml.safe_load(fh.read())
                self.assertEqual(
                    data['environments'].keys(), ['linode'])
                self.assertEqual(
                    data['environments']['linode']['bootstrap-host'],
                    '1.1.1.1')

            with open(os.path.join(env['JUJU_HOME'],
                                   'environments', 'linode.jenv'), 'w') as fh:
        ...
        self.assertNotIn('boot-linode', os.listdir(juju_home))
