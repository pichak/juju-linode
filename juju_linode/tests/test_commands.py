...
from juju_linode.commands import (
    BaseCommand,
    Bootstrap,
    AddMachine,
    TerminateMachine,
...
from juju_linode.client import SSHKey, Linode_instace
from juju_linode.exceptions import ConfigError
from juju_linode.tests.base import Base

# Generated from constraints.images(linode_client)
IMAGE_MAP = {
    'precise': 5588928,
    '12.04': 5588928,
    '14.04': 5141286,
...
class CommandBase(Base):
    ...
    def setUp(self):
        ...
        self.output = self.capture_logging('juju.linode')
    ...
    def setup_env(self, conf=None):
        ...
        with tempfile.NamedTemporaryFile(delete=False) as f:
            ...
            self.config.get_env_name.return_value = 'linode'
            if conf is None:
                conf = {
                    'environments': {
                        'linode': {
...
class BaseCommandTest(CommandBase):
    ...
    def test_check_preconditions_no_named_env(self):
        ...
        try:
            ...
            self.assertIn(
                "Environment 'linode' not in environments.yaml", str(e))
...
class BootstrapTest(CommandBase):
    ...
    @mock.patch('juju_linode.constraints.get_images')
    @mock.patch('juju_linode.ops.ssh')
    def test_bootstrap(self, mock_ssh, mock_get_images):
        ...
        self.provider.get_instance.return_value = Linode_instace.from_dict(dict(
            id=2121,
            name='linode-13290123j13',
            ip_address="10.0.2.1"))
        self.cmd.run()
...
class DestroyEnvironmentTest(CommandBase):
    ...
    def test_destroy_environment_force(self):
        ...
        self.provider.get_instances.return_value = [
            Linode_instace.from_dict(dict(
                id=220, name="linodeabc", ip_address="10.0.1.19")),
            Linode_instace.from_dict(dict(
                id=221, name="linode-123123", ip_address="10.0.1.23")),
            Linode_instace.from_dict(dict(
                id=258, name="linode-209123", ip_address="10.0.1.25")),
            Linode_instace.from_dict(dict(
    ...
    @mock.patch('juju_linode.commands.time')
